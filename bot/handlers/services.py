"""
Services section: list grave cleaning services, add to cart.
"""
...
async def show_services(message: Message) -> None:
    ...
    if not services:
        ...
        return
    text = get_text(lang, "section_grave_cleaning") + "\n\n"
    text += "\n".join(
        f"• {s.get_name(lang)} — {format_price(s.price, lang)}\n  {s.get_description(lang)[:80]}..."
        for s in services
    )
    await message.answer(
        text,
...
async def continue_shopping_services_callback(callback: CallbackQuery) -> None:
    ...
    Edits the notification to show cleaning catalog again.
    """
    await callback.answer()
    lang = await _get_lang(callback.from_user.id)
    async with async_session_factory() as session:
...
# -----------------------------------------------------------------------------
# Service order: single order, auto redirect to payment (no checkout form)
# -----------------------------------------------------------------------------
